...
\color{violet}
Given that ground state wave function, the expectation value would need to be calculated first
\begin{gather*}
\braket{\psi_{m}^{0} | H' | \psi_{1}^{0}} = \int_{0}^{L} \sqrt{\frac{2}{L}} \sin\left( \frac{m\pi x}{L} \right) V_{0} \sqrt{\frac{2}{L}} \sin\left( \frac{\pi x}{L} \right) dx = \frac{2}{L} V_{0} \int_{0}^{L} \sin\left( \frac{m\pi x}{L} \right) \sin\left( \frac{\pi x}{L} \right) dx \\
= \frac{2}{L} V_{0} \int_{0}^{L} \frac{1}{2} \left[ \cos \left( \frac{\pi x}{L}(m - 1) \right) + \cos\left( \frac{\pi x}{L}(m + 1) \right) \right]dx = \frac{V_{0}}{\pi} \left\{ \frac{\sin\left[ \frac{\pi x}{L}(m - 1) \right]}{m - 1} + \frac{\sin\left[ \frac{\pi x}{L}(m  + 1) \right]}{m + 1} \right\}
\end{gather*}
Given that, $\psi_{1}^{1}(x)$ is
\begin{gather*}
\psi_{1}^{1}(x) = \sum_{m \neq 1} \frac{\frac{V_{0}}{\pi} \left\{ \frac{\sin\left[ \frac{\pi x}{L}(m - 1) \right]}{m - 1} + \frac{\sin\left[ \frac{\pi x}{L}(m  + 1) \right]}{m + 1} \right\}}{\frac{\hbar^{2}\pi^{2}}{2mL^{2}}(1^{2} - m^{2})}\sqrt{\frac{2}{L}} \sin\left( \frac{m\pi x}{L} \right)
\end{gather*}
\color{black}
...
\color{violet}
The Taylor series expansion of $\cos(x)$ is $1 - \frac{x^{2}}{2!} + \frac{x^{4}}{4!} + \dots$. So the series expansion of $\cos(2\pi K_{J}\varphi)$ is
\begin{gather*}
\cos(2\pi K_{J}\varphi) \approx 1 - \frac{(2\pi K_{J}\varphi)^{2}}{2} + \frac{(2\pi K_{J}\varphi)^{4}}{24} = 1 - 2\pi^{2}K_{J}^{2}\varphi^{2} + \frac{16}{24}\pi^{4}K_{J}^{4}\varphi^{4}
\end{gather*}
So the Hamiltonian would become
\begin{gather*}
\hat{H} \approx \frac{q^{2}}{2C} - E_{J}\cos(2\pi K_{J}\varphi) = \frac{q^{2}}{2C} - E_{J} + 2\pi^{2}K_{J}^{2}\varphi^{2}E_{J} - \frac{2}{3}\pi^{4}K_{J}^{4}\varphi^{4}E_{J}
\end{gather*}
\color{black}
...
\color{violet}
The most analogous terms from the result above are $\frac{q^{2}}{2C}$ and $2\pi^{2}K_{J}^{2}\varphi^{2}E_{J}$. So that would be unperturbed term, since it corresponds most to the quantum LC circuit's Hamiltonian. The perturbation term $\hat{H}'$ would be $-E_{j} - \frac{2}{3}\pi^{4}K_{J}^{4}\varphi^{4}E_{J}$. The effecitve capacitance $L_{\text{eff}}$ would then be
\begin{gather*}
\frac{1}{2L} = 2\pi^{2}K_{J}^{2}E_{J} \rightarrow L_{\text{eff}} = \frac{1}{4\pi^{2}K_{J}^{2}E_{J}}
\end{gather*}
\color{black}
...
\color{violet}
The perturbing term, is defined as $-E_{J} - \frac{2}{3}\pi^{4}K_{J}^{4}\varphi^{4}E_{J}$. Given that $\hat{\varphi} = \frac{i}{\pi K_{J}}^{\frac{1}{4}}(\hat{a}_{+} - \hat{a}_{-})$, the Hamiltonian could be rewritten as
\begin{gather*}
E_{n}^{(1)} = -E_{J} - \frac{2}{3}\pi^{4}K_{J}^{4}\left[ \frac{i}{\pi K_{J}} \left( \frac{E_{C}}{8E_{J}} \right)^{\frac{1}{4}}(\hat{a}_{+} - \hat{a}_{-}) \right]^{4} E_{J} = -E_{J} - \frac{2}{3}\pi^{4}K_{J}^{4} \frac{i^{4}}{\pi^{4}K_{J}^{4}} \left( \frac{E_{C}}{8E_{J}} \right) (\hat{a}_{+} - \hat{a}_{-})^{4} \\
= -E_{J} - \frac{2}{24} E_{C} (\hat{a}_{+} - \hat{a}_{-})^{4} = -E_{J} - \frac{1}{12} E_{C}(6n^{2} + 6n + 3)
\end{gather*}
\color{black}
...
In this problem, we will explore the transition rates between the ground state $\psi_{100}$ and the 4-fold degenerate first excited states $\psi_{2lm}$ of a hydrogen-like atom. The electromagnetic interaction is provided by a z-polarized, time-dependent electric field $\vec{E} = E(t)\hat{z}$.
...
\color{violet}
For $\hat{H}_{100, 100}'$, the transition energy would be 0, since there's no transition happening. For $\hat{H}_{200, 100}$, the transition energy is
\begin{gather*}
\braket{\psi_{200} | \hat{H}' | \psi_{100}} = \braket{\psi_{200} | eE(t)z | \psi_{100}} = eE(t)z \braket{\psi_{200} | z | \psi_{100}} = 0
\end{gather*}
Along the z-axis, both wavefunctions are symmetric, so the end result is 0. For $\hat{H}_{210, 100}$, the transition energy is
\begin{gather*}
	content...
\end{gather*}
\color{black}
